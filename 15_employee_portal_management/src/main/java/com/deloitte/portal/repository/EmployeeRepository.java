...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
	...
	@Query("select emp from employees emp where emp.gender= :gender and (emp.firstName like %:name% or emp.lastName like %:name%)")
	List<Employee> searchEmployeeByName(String gender, String name);
	
	@Query("select count(*) from employees")
	int NoOfRecords();
	
	@Query("select emp from employees emp where emp.gender= :gender")
