...
public class PortalController {
	...
	public ModelAndView save(@Valid @ModelAttribute("employee") Employee employee, BindingResult br) {
		...
		try {
			List<Employee> emp =employeeService.searchEmployeeById(employee.getEmpId());
			if(emp == null) {
				employeeService.save(employee);
				String message = "Employee record has been saved successfully";
				return home().addObject("successMessage", message);
			}
			String[] message= {"Emp Id should be unique!","Form has invalid input data, please correct the errors and submit again!"};
			return new ModelAndView("errorpage","empIderror",message);
	...
	@RequestMapping(value = "/input/edit", method = RequestMethod.POST)
	public ModelAndView saveAgain(@Valid @ModelAttribute("employee") Employee employee, BindingResult br) {
		if (br.hasErrors()) {
			String message = "Form has invalid input data, please correct the errors and submit again!";
			return new ModelAndView("errorpage-edit", "errorMessage", message);
		}
		try {
				employeeService.save(employee);
				String message = "Employee record has been saved successfully";
			...
			if(e.getMessage().contains("email_id")) {
				String[] message= {"Email is already taken","Form has invalid input data, please correct the errors and submit again!"};
				return new ModelAndView("errorpage-edit","emailMsg",message);
			}
			String[] message= {"Email is already taken","Form has invalid input data, please correct the errors and submit again!"};
			return new ModelAndView("errorpage-edit","officeemailMsg",message);
		}
	...
	public String searchEmployeebyId(Model model, @Param("id") String id, @Param("gender") String gender,
		...
		if(id.isBlank()) {
			...
			if(id.matches(regex)) {
				...
				List<Employee> employee = employeeService.searchEmployeeById(empid);
				if (employee == null) {
					String msg = "Sorry, No Employee records found!";
					model.addAttribute("message", msg);
					return "home";
					...
					Integer count= employee.size();
					model.addAttribute("recordsCount", count);
					return "home";
				}
			}
			String message= "Employee Id should be numeric value!";
			model.addAttribute("empIderror", message);
			return "home";
